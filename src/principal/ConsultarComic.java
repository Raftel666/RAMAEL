...
public class ConsultarComic extends JDialogMethods implements ActionListener, KeyListener, MouseListener{
    ...
    public ConsultarComic(Frame owner, boolean modal) {
        super(owner, modal);
        addWindow(null, 800,600,"Consultar Cómic", false, this);
    ...
    private void llenarTablaPeliculas() {
        try {
            ...
            String SQL = "select * from comics order by idComics asc";
    ...
    private void buscarFrase() {
        try {
            PreparedStatement BuscarStm;
            String Sql = "select * from comics where Titulo like '%"+txtConsultar.getText()+"%'";
            BuscarStm = Conex.MiConexion.getConexion().prepareCall(Sql);
            ResultSet RsBuscar = BuscarStm.executeQuery();
            ResultSetMetaData RsMD = RsBuscar.getMetaData();
            int numeroDeColumnas = RsMD.getColumnCount();
