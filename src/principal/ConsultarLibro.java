...
public class ConsultarLibro extends JDialogMethods implements ActionListener, KeyListener, MouseListener{
    ...
    public ConsultarLibro(Frame owner, boolean modal) {
        super(owner, modal);
        addWindow(null, 800,600,"Consultar Libros", false, this);
    ...
    private void llenarTablaPeliculas() {
        try {
            ...
            String SQL = "select * from libros order by ISBN asc";
    ...
    private void buscarFrase() {
        try {
            PreparedStatement BuscarStm;
            String Sql = "select * from libros where Titulo like '%"+txtConsultar.getText()+"%'";
            BuscarStm = Conex.MiConexion.getConexion().prepareCall(Sql);
            ResultSet RsBuscar = BuscarStm.executeQuery();
            ResultSetMetaData RsMD = RsBuscar.getMetaData();
            int numeroDeColumnas = RsMD.getColumnCount();
